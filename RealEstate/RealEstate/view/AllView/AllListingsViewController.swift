...
//import SDWebImage
//
//class AllListingsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
//
...
//        let detailViewController = AllListingsDetailViewController()
...
class AllListingsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private let databaseReference = Database.database().reference(fromURL: "https://realestate-eb7db-default-rtdb.firebaseio.com/")
    private var allListingIds: [String] = []
    private var allListingData: [RealEstateListing] = []
    ...
    override func viewDidLoad() {
        ...
        let allListingsRef = databaseReference.child("listings")
        allListingsRef.observe(.value) { snapshot in
            guard let snapshots = snapshot.children.allObjects as? [DataSnapshot] else {
                print("No listings found")
                return
            }

            self.allListingData.removeAll()
            self.allListingIds.removeAll()
            ...
            for snap in snapshots {
                ...
                self.allListingIds.append(listingId)
                self.allListingData.append(listing)
            }
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return allListingData.count
    }

    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = UITableViewCell(style: .default, reuseIdentifier: nil)
        let listing = allListingData[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let detailViewController = AllListingsDetailViewController()
        detailViewController.listing = allListingData[indexPath.row]
        navigationController?.pushViewController(detailViewController, animated: true)
    }
